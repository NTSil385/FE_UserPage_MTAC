      ...
      services: [
        {
          ...
          description:
            "Giải pháp xử lý chất thải nguy hại an toàn và đúng quy định",
          icon: "bi-radioactive",
          features: [
            "Thu gom tận nơi",
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function renderServiceGrids() {
    Object.values(serviceConfig.categories).forEach((category) => {
      ...
      category.services.forEach((service, index) => {
        ...
        card.innerHTML = `
                    <a href="#" class="select-btn" data-service-id="${
                      service.id
                    }">
    ...
    Object.values(serviceConfig.categories).forEach((category) => {
      category.services.forEach((service) => {
        ...
        form.innerHTML = `
                    ...
                    <form id="${service.id}Form" data-service-type="${
          service.id
        }">
                          ...
                          .map(
                            ...
                            <div class="form-section">
                                ...
                                <div class="form-row">
                                    ${section.fields
                                      .map((field) =>
                                        renderFormField(field, service.id)
                                      )
                                      .join("")}
                                </div>
                            </div>
  ...
  function handleCategoryClick(e) {
    ...
    const visibleForm = document.querySelector(
      '.quotation-form[style*="display: block"]'
    );
    const visibleResult = document.querySelector(
    ...
    if (visibleForm || visibleResult) {
      ...
      if (visibleForm.dataset.categoryId !== categoryId) {
        ...
        document.querySelectorAll(".services-grid").forEach((grid) => {
          grid.style.display =
            grid.id === `${categoryId}-services` ? "grid" : "none";
        });
      ...
      document.querySelectorAll(".services-grid").forEach((grid) => {
        grid.style.display =
          grid.id === `${categoryId}-services` &&
          category.classList.contains("expanded")
            ? "grid"
            : "none";
      });
    }
  ...
  function handleDropdownItemClick(e) {
    ...
    if (category) {
      const dropdownItems = category.querySelectorAll(
  ...
  function createPagination(totalItems, categoryId) {
    ...
    pagination
      ...
      .addEventListener("click", () => changePage(categoryId, "prev"));
    pagination
      .querySelector(".next-page")
      .addEventListener("click", () => changePage(categoryId, "next"));
  ...
  function updatePageDisplay(categoryId, pageNumber) {
    ...
    cards.forEach((card) => {
      card.style.display =
        parseInt(card.dataset.page) === pageNumber ? "block" : "none";
    });

    // Cập nhật UI pagination
    const pageInfo = document.querySelector(
      `.page-info[data-category="${categoryId}"] .current-page`
    );
    if (pageInfo) {
