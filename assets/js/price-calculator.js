...
export function calculatePrice(formData, serviceConfig) {
  ...
  if (priceCalculator.type === "environmental_service") {
    ...
    const total = Object.entries(priceCalculator.components).reduce(
      (sum, [component, isEnabled]) => {
  ...
  const priceRule =
    totalWeight <= (provinceConfig.weightThreshold || 600)
...
function getTotalWeight(formData, calculator) {
  if (calculator.type === "simple") {
    ...
    return Object.values(formData.waste_details || {}).reduce(
